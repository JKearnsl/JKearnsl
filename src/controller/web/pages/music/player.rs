...
fn Controls(idx: RwSignal<usize>, playing: RwSignal<bool>, cur: RwSignal<u32>) -> AnyView {
    let go_prev = move |_: leptos::ev::MouseEvent| idx.update(|i| *i = (*i + crate::controller::web::pages::music::TRACKS.len() - 1) % crate::controller::web::pages::music::TRACKS.len());
    let go_next = move |_: leptos::ev::MouseEvent| idx.update(|i| *i = (*i + 1) % crate::controller::web::pages::music::TRACKS.len());
    let toggle  = move |_: leptos::ev::MouseEvent| playing.update(|p| *p = !*p);
    let track_dur = move || crate::controller::web::pages::music::TRACKS[idx.get()].dur;
